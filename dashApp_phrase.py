...
app.layout = dbc.Container([ #dbc rows and col where number of columns and rows are determined by how many rows and columns are in the parameters
    dbc.Row([ #e.g. row(col col col) <-- 3 columns || row (col) <-- 1 column https://dash-bootstrap-components.opensource.faculty.ai/docs/components/layout/
        dbc.Col([
            ...
            dcc.Input(
                ...
                value = 'dust, dispense blade, debris', #setup inital values to put in
                style = {'width': '100%'}
            ),
            dbc.Button(
                id = 'submit-button',
    ...
    dbc.Row([
        dbc.Col([
            dcc.Graph(id='keyword-graph') #populate graph based on the keyword-input which changes the value of the placeholder
        ]),
    ]),
])
...
def update_graph(n_clicks, keywords):  
    ...
    keyword_list = []
    #loop through keywords that are split by ','#
    for kw in keywords.split(','): 
        keyword_list.append(kw.strip().lower())

    #keyword_list = [kw.strip().lower() for kw in keywords.split(',')]

    #apply the function here which applies strip() and lower() while splitting by ','
    df['keyword_frequency'] = df['combined_text'].apply(lambda x: count_keywords(x, keyword_list))
